  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
      <div className="relative flex-1">
        ...
        <Input placeholder="Поиск ассетов..." className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)} />
        {search && (
      ...
      <div className="flex gap-2">
        <Button variant={filter.view === 'table' ? 'default' : 'outline'} size="sm" onClick={() => setFilter({ view: 'table' })}>Таблица</Button>
        <Button variant={filter.view === 'grid' ? 'default' : 'outline'} size="sm" onClick={() => setFilter({ view: 'grid' })}>Сетка</Button>
      </div>
    </div>
  )
}
